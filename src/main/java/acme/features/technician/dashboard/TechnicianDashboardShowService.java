...
@Service
@GuiService
public class TechnicianDashboardShowService extends AbstractGuiService<Technician, TechniciansDashboards> {
	...
	@Override
	public void load() {
		final TechniciansDashboards dashboard = new TechniciansDashboards();
		...
		if (technician != null) {
			...
			if (technician.getId() != 0) {
				pendingRecords = this.repository.countMaintanenceRecordsByStatus(technician.getId(), StatusMaintanenceRecord.PENDING).orElse(0);
				inProgressRecords = this.repository.countMaintanenceRecordsByStatus(technician.getId(), StatusMaintanenceRecord.IN_PROGRESS).orElse(0);
				completedRecords = this.repository.countMaintanenceRecordsByStatus(technician.getId(), StatusMaintanenceRecord.COMPLETED).orElse(0);
			}
			...
			MaintanenceRecord nearestRecord = null;
			...
			if (technician.getId() != 0) {
				...
				if (nearestRecord != null)
					nearestRecordDate = nearestRecord.getNextMaintanence();
			}
	...
	@Override
	public void unbind(final TechniciansDashboards object) {
